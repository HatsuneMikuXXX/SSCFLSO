...
void SSCFLSOUnitTest::test2(){
	...
	const bool c1 = areSame(SSCFLSOObject.demands, demands);
	const bool c2 = areSame(SSCFLSOObject.capacities, capacities); 
	const bool c3 = areSame(SSCFLSOObject.facility_costs, facility_costs); 
	const bool c4 = areSame(SSCFLSOObject.distribution_costs, distribution_costs);
	if(!(c1 && c2 && c3 && c4)){
	...
	SSCFLSOObject = GeneratorObject.load_instance("instances/unit_tests/unit_test_SSCFLSO.plc");
	const bool d1 = areSame(SSCFLSOObject.demands[0], 1.25);
	if(!d1){
...
void SSCFLSOUnitTest::test3(){
	...
	const bool value1 = areSame(FLV.value(), 6.9);
	...
	const bool value2 = areSame(FLV.value(), 6.99);
	const bool feasible2 = FLV.feasible();
	const bool violation2 = FLV.exceeds_capacity() == client_facility_assignment{0, 1, 0};
	FLV.set_solution({1, 0, 0});
	const bool assign3 = FLV.get_assignment() == client_facility_assignment{0, 0, 0};
	const bool value3 = areSame(FLV.value(), 6.9);
	...
	const bool value4 = areSame(FLV.value(), 8.82);
